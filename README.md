...
### Optional List Store personal-list authority

Clockwork keeps JSON-file list storage by default. To move the To Do, Watch,
Read, Listen, and Invest compatibility routes onto List Store's local SQLite
authority, install the reviewed `casonk-list-store` package in Clockwork's
...
revision-guarded List Store resources. Leave both unset to retain the legacy
...
List Store adapter's validated import of existing JSON before enabling the
